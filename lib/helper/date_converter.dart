...
class DateConverter {
  ...
  static String compareDates(String inputDate) {
    ...
    if (hoursDifference < 1) {
      ...
      return '$hoursDifference ${getTranslated("hr_ago", Get.context!)}';
  ...
  static String getRelativeDateStatus(String inputDate, BuildContext context) {
    try {
      ...
      if (difference == 0) {
        return getTranslated("today", context) ?? '';
      } else if (difference == 1) {
        return getTranslated("yesterday", context) ?? '';
      } else {
        return DateFormat('MM/dd/yyyy').format(parsedDate);
      }
    } catch (e) {
      return getTranslated("invalid_date", context) ?? ''; // Localized "Invalid date"
    }
  }

  static String formatDateWithCommaAnd24Hour(DateTime dateTime) {
  ...
  /// Calculates the claim deadline for an auction winner.
  /// [claimStartTime] is the ISO string from `my_bid.claim_start_time`.
  /// [limitValue] and [limitUnit] come from config's `auction_winner_claim_time_limit`.
  /// Returns null if inputs are invalid or missing.
  static DateTime? auctionClaimDeadline({
    required String? claimStartTime,
    required int? limitValue,
    required String? limitUnit,
  }) {
    final start = DateTime.tryParse(claimStartTime?.trim() ?? '');
    if (start == null || (limitValue ?? 0) <= 0) return null;

    final unit = limitUnit?.toLowerCase().trim() ?? '';

    final Duration duration = unit == 'day' || unit == 'days'
        ? Duration(days: limitValue!)
        : unit == 'hour' || unit == 'hours'
        ? Duration(hours: limitValue!)
        : Duration(minutes: limitValue!);
