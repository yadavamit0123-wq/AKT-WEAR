...
import 'package:flutter_sixvalley_ecommerce/features/ai_shopping/domain/repository/ai_shopping_repository.dart';
import 'package:flutter_sixvalley_ecommerce/features/ai_shopping/domain/repository/ai_shopping_repository_interface.dart';
import 'package:flutter_sixvalley_ecommerce/features/ai_shopping/domain/services/ai_shopping_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_ai/domain/repository/auction_ai_repository.dart';
import 'package:flutter_sixvalley_ecommerce/features/auction_ai/domain/repository/auction_ai_repository_interface.dart';
import 'package:flutter_sixvalley_ecommerce/features/auction_ai/domain/services/auction_ai_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction/domain/repository/customer_auction_list_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction/domain/service/customer_auction_list_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_details/domain/repositories/creator/creator_auction_details_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_details/domain/repositories/participator/auction_participation_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_details/domain/repositories/participator/participation_auction_details_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_details/domain/services/creator/creator_auction_details_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_details/domain/services/participator/auction_participation_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_details/domain/services/participator/participation_auction_details_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_home/domain/repositories/auction_home_repo.dart';
import 'package:flutter_sixvalley_ecommerce/features/auction_home/domain/repositories/auction_home_repo_interface.dart';
import 'package:flutter_sixvalley_ecommerce/features/auction_home/domain/services/auction_home_service.dart';
import 'package:flutter_sixvalley_ecommerce/features/auction_home/domain/services/auction_home_service_interface.dart';
import 'package:flutter_sixvalley_ecommerce/features/auction_category/domain/repositories/auction_category_repo.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_category/domain/services/auction_category_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_list/domain/repository/auction_product_queue_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_list/domain/services/auction_product_queue_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_search/domain/repositories/auction_search_repo.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_search/domain/services/auction_search_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_transaction/domain/repository/auction_transaction_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_checkout/domain/repositories/auction_checkout_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/auction_checkout/domain/services/auction_checkout_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/create_auction/domain/repository/add_auction_product_repository.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/create_auction/domain/services/add_auction_product_service.dart';
...
import 'package:flutter_sixvalley_ecommerce/features/vat_tax/domain/repository/vat_tax_repository.dart';
import 'package:flutter_sixvalley_ecommerce/features/vat_tax/domain/repository/vat_tax_repository_interface.dart';
import 'package:flutter_sixvalley_ecommerce/features/vat_tax/domain/service/vat_tax_service.dart';
...
import 'features/auction_dashboard_summary/domain/repository/auction_dashboard_summary_repository.dart';
import 'features/auction_dashboard_summary/domain/repository/auction_dashboard_summary_repository_interface.dart';
import 'features/auction_dashboard_summary/domain/services/auction_dashboard_summary_service.dart';
import 'features/auction_dashboard_summary/domain/services/auction_dashboard_summary_service_interface.dart';
...
import 'features/transaction/domain/repository/transaction_repository_interface.dart';
import 'features/transaction/domain/service/transaction_service.dart';
import 'features/transaction/domain/service/transaction_service_interface.dart';
import 'features/user_created_auction_list/controllers/user_created_auction_list_controller.dart';
import 'features/user_created_auction_list/domain/repository/user_created_auction_list_repository.dart';
import 'features/user_created_auction_list/domain/services/user_created_auction_list_service.dart';
import 'features/user_created_auction_list/domain/services/user_created_auction_list_service_interface.dart';
...
Future<void> init() async {
  ...
  sl.registerLazySingleton(() => AuctionCategoryRepository(dioClient: sl(), dataSyncRepoInterface: sl()));
  sl.registerLazySingleton(() => AuctionHomeRepository(dioClient: sl(), dataSyncRepoInterface: sl()));
  ...
  sl.registerFactory(() => AuctionCheckoutController(auctionCheckoutServiceInterface: sl()));
  ...
  AuctionCheckoutRepositoryInterface auctionCheckoutRepositoryInterface = AuctionCheckoutRepository(dioClient: sl());
  sl.registerLazySingleton(() => auctionCheckoutRepositoryInterface);
  AuctionCheckoutServiceInterface auctionCheckoutServiceInterface = AuctionCheckoutService(auctionCheckoutRepositoryInterface: sl());
  ...
  AuctionCategoryRepoInterface auctionCategoryRepoInterface = AuctionCategoryRepository(dioClient: sl(), dataSyncRepoInterface: sl());
  sl.registerLazySingleton(() => auctionCategoryRepoInterface);
  AuctionCategoryServiceInterface auctionCategoryServiceInterface = AuctionCategoryService(auctionCategoryRepoInterface: sl());
  sl.registerLazySingleton(() => auctionCategoryServiceInterface);
  sl.registerLazySingleton(() => AuctionCategoryService(auctionCategoryRepoInterface : sl()));

  AuctionHomeRepoInterface auctionHomeRepoInterface = AuctionHomeRepository(dioClient: sl(), dataSyncRepoInterface: sl());
  sl.registerLazySingleton(() => auctionHomeRepoInterface);
  AuctionHomeServiceInterface auctionHomeServiceInterface = AuctionHomeService(auctionHomeRepoInterface: sl());
  sl.registerLazySingleton(() => auctionHomeServiceInterface);
  sl.registerLazySingleton(() => AuctionHomeService(auctionHomeRepoInterface : sl()));

  sl.registerLazySingleton(() => AuctionSearchRepository(dioClient: sl(), sharedPreferences: sl()));
  AuctionSearchRepoInterface auctionSearchRepoInterface = AuctionSearchRepository(dioClient: sl(), sharedPreferences: sl());
  sl.registerLazySingleton(() => auctionSearchRepoInterface);

  AuctionSearchServiceInterface auctionSearchServiceInterface = AuctionSearchService(auctionSearchRepoInterface: sl());
  sl.registerLazySingleton(() => auctionSearchServiceInterface);

  sl.registerFactory(() => AuctionSearchController(auctionSearchServiceInterface: sl()));

  sl.registerLazySingleton(() => AddAuctionProductMediaController());

  AddAuctionProductRepositoryInterface addAuctionProductRepositoryInterface = AddAuctionProductRepository(dioClient: sl());
  sl.registerLazySingleton(() => addAuctionProductRepositoryInterface);

  AddAuctionProductServiceInterface addAuctionProductServiceInterface = AddAuctionProductService(auctionRepoInterface: sl());
  sl.registerLazySingleton(() => addAuctionProductServiceInterface);

  sl.registerFactory(() => AddAuctionProductController(addAuctionProductServiceInterface: sl()));

  sl.registerFactory(() => AuctionProductQueueController(auctionProductQueueServiceInterface: sl()));

  AuctionProductQueueRepositoryInterface repoInterface = AuctionProductQueueRepository(dioClient: sl());
  sl.registerLazySingleton(() => repoInterface);

  AuctionProductQueueServiceInterface serviceInterface = AuctionProductQueueService(auctionProductRepoInterface: sl());
  ...
  CreatorAuctionDetailsServiceInterface creatorAuctionDetailsServiceInterface = CreatorAuctionDetailsService(repoInterface: sl());
  sl.registerLazySingleton(() => creatorAuctionDetailsServiceInterface);

  sl.registerFactory(() => CreatorAuctionDetailsController(serviceInterface: sl()));
  ...
  sl.registerFactory(() => CustomerAuctionListController(serviceInterface: sl()));
  ...
  sl.registerFactory(() => AiShoppingController(aiShoppingServiceInterface: sl()));
  ...
  sl.registerFactory(() => AuctionAiController(auctionAiServiceInterface: sl()));

  AuctionParticipationRepositoryInterface auctionParticipationRepositoryInterface =
  ...
  sl.registerFactory(() => AuctionParticipationController(auctionParticipationServiceInterface: sl()));

  sl.registerLazySingleton<ParticipationAuctionDetailsRepositoryInterface>(() => ParticipationAuctionDetailsRepository(dioClient: sl()));

  sl.registerLazySingleton<ParticipationAuctionDetailsServiceInterface>(() => ParticipationAuctionDetailsService(participationAuctionDetailsRepositoryInterface: sl()));

  sl.registerFactory(() => ParticipationAuctionDetailsController(participationAuctionDetailsServiceInterface: sl()));

  UserCreatedAuctionListRepositoryInterface userCreatedAuctionListRepoInterface =
  UserCreatedAuctionListRepository(dioClient: sl());
  sl.registerLazySingleton(() => userCreatedAuctionListRepoInterface);

  UserCreatedAuctionListServiceInterface userCreatedAuctionListServiceInterface =
  UserCreatedAuctionListService(userCreatedAuctionListRepositoryInterface: sl());
  sl.registerLazySingleton(() => userCreatedAuctionListServiceInterface);

  sl.registerFactory(() => UserCreatedAuctionListController(userCreatedAuctionListServiceInterface: sl()));

  AuctionDashboardSummaryRepositoryInterface auctionDashboardSummaryRepo = AuctionDashboardSummaryRepository(dioClient: sl());
  ...
  sl.registerFactory(() => AuctionDashboardSummaryController(serviceInterface: sl()));
  ...
  sl.registerFactory(() => AuctionTransactionController(serviceInterface: sl()));
