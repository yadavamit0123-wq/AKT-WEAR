...
class BrandController extends ChangeNotifier {
  ...
  Future<void> getSellerWiseBrandList(String slug) async {
    ApiResponseModel apiResponse = await brandRepo!.getSellerWiseBrandList(slug);
  ...
  void uncheckSellerBrandList() {
    if(_brandList.isNotEmpty) {
      _brandList.map((brand) {brand.checked = false;}).toList();
    }
  }
