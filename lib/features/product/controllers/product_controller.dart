...
class ProductController extends ChangeNotifier {
  ...
  bool get isHomeDataLoaded => _recommendedProduct != null && _justForYouProductModel != null && _clearanceProductModel != null;

  Future<void> loadHomeDataWithCache() async {
    if (isHomeDataLoaded) return;
    await Future.wait([
      getFeaturedProductModel(1),
      getRecommendedProduct(),
      getJustForYouProduct(1, isUpdate: false),
      getClearanceAllProductList(1, isUpdate: false),
    ]);
  }
  ...
  final Map<int, ProductModel?> _categoryProductMap = {};
  final Set<int> _fetchingCategoryIds = {};

  // Shared across all category tabs so rapid tab switching cancels the pending
  // fetch and only the tab the user settles on actually hits the API.
  final DebounceHelper _categoryProductDebounce = DebounceHelper(milliseconds: 350);

  ProductModel? categoryProductsFor(int categoryId) => _categoryProductMap[categoryId];

  void clearCategoryProductFor(int categoryId) => _categoryProductMap.remove(categoryId);

  void clearCategoryProductCache() {
    _categoryProductMap.clear();
    _fetchingCategoryIds.clear();
  }

  /// Debounced entry point for the first page of a category's products. Used on
  /// tab switch so that swiping quickly through tabs does not fire an API call
  /// for every intermediate tab — only the last requested category is fetched.
  void getCategoryProductsDebounced(int categoryId, {String searchProduct = ''}) {
    // Already loaded or currently loading — nothing to schedule.
    if (_categoryProductMap[categoryId] != null || _fetchingCategoryIds.contains(categoryId)) {
      return;
    }
    _categoryProductDebounce.run(() {
      getCategoryProducts(categoryId, 1, searchProduct: searchProduct);
    });
  }

  Future<void> getCategoryProducts(int categoryId, int offset, {String searchProduct = '', String? productType, String? brandIds, String? authorIds, String? publishingIds}) async {
    if (offset == 1) {
      if (_fetchingCategoryIds.contains(categoryId)) return;

      _fetchingCategoryIds.add(categoryId);
      _categoryProductMap[categoryId] = null;
      notifyListeners();

      final ApiResponseModel apiResponse = await productServiceInterface!
          .getBrandOrCategoryProductList(isBrand: false, id: categoryId, offset: 1, searchProduct: searchProduct, productType: productType, brandIds: brandIds, authorIds: authorIds, publishingIds: publishingIds);

      if (apiResponse.response?.statusCode == 200) {
        _categoryProductMap[categoryId] = ProductModel.fromJson(apiResponse.response?.data);
      } else {
        _categoryProductMap[categoryId] = ProductModel(products: []);
        ApiChecker.checkApi(apiResponse);
      }
      ...
      final ApiResponseModel apiResponse = await productServiceInterface!
          .getBrandOrCategoryProductList(isBrand: false, id: categoryId, offset: offset, searchProduct: searchProduct, productType: productType, brandIds: brandIds, authorIds: authorIds, publishingIds: publishingIds);

      if (apiResponse.response?.statusCode == 200) {
        final parsed = ProductModel.fromJson(apiResponse.response?.data);
        _categoryProductMap[categoryId]?.totalSize = parsed.totalSize;
        _categoryProductMap[categoryId]?.offset = parsed.offset;
        _categoryProductMap[categoryId]?.products?.addAll(parsed.products ?? []);
      } else {
        ApiChecker.checkApi(apiResponse);
      }
      notifyListeners();
  ...
  final Map<ProductType, ProductModel?> _productTypeMap = {};
  final Set<ProductType> _fetchingProductTypes = {};
  final DebounceHelper _productTypeDebounce = DebounceHelper(milliseconds: 350);

  ProductModel? productModelForType(ProductType type) => _productTypeMap[type];

  void clearProductTypeCache() {
    _productTypeMap.clear();
    _fetchingProductTypes.clear();
  }

  void getProductsForTypeDebounced(ProductType type) {
    if (_productTypeMap[type] != null || _fetchingProductTypes.contains(type)) return;
    _productTypeDebounce.run(() {
      getProductsForType(type, 1);
    });
  }

  Future<void> getProductsForType(ProductType type, int offset) async {
    if (offset == 1) {
      if (_fetchingProductTypes.contains(type)) return;

      _fetchingProductTypes.add(type);
      _productTypeMap[type] = null;
      notifyListeners();

      final ApiResponseModel? apiResponse = await productServiceInterface
          ?.getProductModelByType<Response>(offset: 1, productType: type, source: DataSourceEnum.client);

      if (apiResponse?.response?.statusCode == 200) {
        _productTypeMap[type] = ProductModel.fromJson(apiResponse?.response?.data);
      } else {
        _productTypeMap[type] = ProductModel(offset: 1, products: []);
        if (apiResponse != null) ApiChecker.checkApi(apiResponse);
      }

      _fetchingProductTypes.remove(type);
      notifyListeners();
    } else {
      final ApiResponseModel? apiResponse = await productServiceInterface
          ?.getProductModelByType<Response>(offset: offset, productType: type, source: DataSourceEnum.client);

      if (apiResponse?.response?.statusCode == 200) {
        final ProductModel parsed = ProductModel.fromJson(apiResponse?.response?.data);
        _productTypeMap[type]?.totalSize = parsed.totalSize;
        _productTypeMap[type]?.offset = parsed.offset;
        _productTypeMap[type]?.products?.addAll(parsed.products ?? []);
