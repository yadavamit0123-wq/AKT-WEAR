...
class AuctionSavedAddressListScreen extends StatefulWidget {
  final bool fromGuest;
  const AuctionSavedAddressListScreen({super.key, this.fromGuest = false});

  @override
  State<AuctionSavedAddressListScreen> createState() => _AuctionSavedAddressListScreenState();
}

class _AuctionSavedAddressListScreenState extends State<AuctionSavedAddressListScreen> {
              ...
              locationProvider.addressList != null? locationProvider.addressList!.isNotEmpty ?  ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return InkWell(onTap: () {
                    Provider.of<AuctionCheckoutController>(context, listen: false).setAddressIndex(index);
                    ...
                    child: Padding(padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeDefault),
                      child: Container(margin: const EdgeInsets.only(top: Dimensions.paddingSizeSmall),
                        decoration: BoxDecoration(borderRadius: BorderRadius.circular(10),
                          ...
                          border: index == Provider.of<AuctionCheckoutController>(context).addressIndex ?
              ...
              Padding(padding: EdgeInsets.only(top: MediaQuery.of(context).size.height/3),
                child: Center(child: Container(alignment: Alignment.center,
                    ...
                    child: const NoInternetOrDataScreenWidget(isNoInternet: false,
                        message: 'no_address_found', icon: Images.noAddress)))) : const AddressShimmerWidget(shrinkWrap: true, physics: NeverScrollableScrollPhysics()),
              ],
            ),
          );
        },
