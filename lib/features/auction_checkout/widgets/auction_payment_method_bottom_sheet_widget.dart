...
import 'package:flutter_sixvalley_ecommerce/features/auction_checkout/widgets/auction_custom_check_box_widget.dart';
import 'package:flutter_sixvalley_ecommerce/features/profile/controllers/profile_contrroller.dart';
import 'package:provider/provider.dart';

class AuctionPaymentMethodBottomSheetWidget extends StatefulWidget {
  final bool onlyDigital;
  final double totalAmount;
  const AuctionPaymentMethodBottomSheetWidget({
    super.key,
    required this.onlyDigital,
    required this.totalAmount,
  });
  @override
  AuctionPaymentMethodBottomSheetWidgetState createState() => AuctionPaymentMethodBottomSheetWidgetState();
}

class AuctionPaymentMethodBottomSheetWidgetState extends State<AuctionPaymentMethodBottomSheetWidget> {
  ...
  AuctionCheckoutController checkoutController = Provider.of<AuctionCheckoutController>(Get.context!, listen: false);
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<AuctionCheckoutController>(builder: (context, checkoutController, _) {
      return PopScope(
        onPopInvokedWithResult: (_, __) {},
        child: Container(constraints: BoxConstraints(maxHeight: MediaQuery.of(context).size.height * 0.7, minHeight: MediaQuery.of(context).size.height * 0.3),
          padding: const EdgeInsets.all(Dimensions.paddingSizeDefault),
          decoration: BoxDecoration(color: Theme.of(context).highlightColor, borderRadius: const BorderRadius.only(topLeft: Radius.circular(20), topRight: Radius.circular(20))),
          child: Column(
                    ...
                    _isPaymentMethodsAvailable(Get.context!, checkoutController.offlinePaymentModel?.offlineMethods)
                        ? Column(
                            ...
                            children: [
                              Row(children: [
                                if (configModel?.walletStatus == 1 && Provider.of<AuthController>(context, listen: false).isLoggedIn())
                                  Consumer<ProfileController>(builder: (context, profileController, _) {
                                    bool isInsufficientBalance = (profileController.balance ?? 0) < widget.totalAmount;
                                    return Expanded(
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          ...
                                          if (isInsufficientBalance)
                                            Padding(
                                              ...
                                              child: Text(
                                                getTranslated('insufficient_balance', context) ?? 'Insufficient Balance',
                                                style: textRegular.copyWith(color: Theme.of(context).colorScheme.error, fontSize: Dimensions.fontSizeExtraSmall),
                                              ),
                                            ),
                              ...
                              if (configModel?.cashOnDelivery == true)
                                Row(children: [
                                  Expanded(
                                    child: CustomButton(
                                        ...
                                        Consumer<SplashController>(builder:
                                          ...
                                          return ListView.separated(
                                            ...
                                            itemBuilder: (context, index) {
                                              return AuctionCustomCheckBoxWidget(
                                                index: index,
                                                padding: 0,
                                        ...
                                        child: Padding(
                                            ...
                                            child: Container(
                                              ...
                                              child: Row(children: [
                                                ...
                                                Text('${getTranslated('pay_offline', context)}',
                                                    ...
                                                    child: Padding(
                                                      ...
                                                      child: Container(
                                                        ...
                                                        child: Padding(padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeDefault),
                                                          child: Center(
                                                              child: Text(checkoutController.offlinePaymentModel?.offlineMethods?[index].methodName ?? '')),
                                                        ),
                                                      ),
                                                    ),
                                                  );
...
bool _isPaymentMethodsAvailable(
  ...
  bool isCODOn = configModel?.cashOnDelivery == true;

  return isWalletOn || isOnlinePaymentMethodsOn || isOfflinePaymentMethodsOn || isCODOn;
}
