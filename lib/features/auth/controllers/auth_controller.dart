...
class AuthController with ChangeNotifier {
  ...
  Future<void> socialLogin(SocialLoginModel socialLogin, Function callback, String? fromPage, VoidCallback? onLoginSuccess) async {
    ...
    if (apiResponse.response != null && apiResponse.response?.statusCode == 200) {
      ...
      if(token != null && token.isNotEmpty) {
        ...
        await authServiceInterface.updateDeviceToken();
        setCurrentLanguage(Provider.of<LocalizationController>(Get.context!, listen: false).getCurrentLanguage() ?? 'en');
        Provider.of<AiShoppingController>(Get.context!, listen: false).clearSession();
  ...
  // Clears all user-scoped state after sign-out or account deletion and routes to login.
  void resetAllUserState(BuildContext context) {
    ...
    RouterHelper.getLoginRoute(
      ...
      onLoginSuccess: () => RouterHelper.getMoreScreenRoute(action: RouteAction.push),
    );
  }
